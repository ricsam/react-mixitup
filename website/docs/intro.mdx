...
If falsy only unmeasured frames and the previous frame will be rendered.
...
Thus by default only the previous frame is measured ? (or maybe the rendered frame should be rendered)
