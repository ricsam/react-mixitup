...
<Tabs>
<TabItem value="numcells" label="Toggle num cells">

```tsx live
function NumCells() {
  const [keys, setKeys] = React.useState(() => range(5));

  const _toggleNumCells = () => {
    setKeys(range(keys.length === 5 ? 10 : 5))
  };
  ...
  return (
    <div style={{position: 'relative'}} >
      <button onClick={_toggleNumCells} style={{ margin: 4 }}>Random</button>
      <ReactMixitup
        keys={keys}
...
This shows how to debug the frames which are next up to be rendered.
